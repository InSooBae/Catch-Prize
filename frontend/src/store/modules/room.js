...
const room = {
  ...
  mutations: {
    ...
    SET_ROOM_MESSAGES: (state, roomMessages) =>
  ...
  actions: {
    fetchRooms({ commit, getters }) {
      fetchRooms(getters.authHeader).then((res) => {
        commit("SET_ROOMS", res.data);
      });
    },

    createRoom({ commit, getters, dispatch }, gameinfo) {
      createRoom(getters.authHeader, gameinfo).then((res) => {
    ...
    enterRoom({ commit, getters, dispatch }, roomId) {
      fetchRoomById(getters.authHeader, roomId).then((res) => {
        console.log(res);
        commit("SET_ROOM", res.data);
        dispatch("subscribeRoom");
    ...
    removeUser({ getters }, data) {
      removeUser(getters.authHeader, data.roomId, data.ovdata).then((res) => {
